...
```{r stats-and-summaries, include = FALSE}
...
This model represents the integration of the four features from the cue allowing each to have their own weight in the decision process (model 15 in Fig. \ref{fig:ch2-model-selection}, which is equivalent to WADD). 
...
We also created 14 models that represent different integration of features: four single parameter models, which represent the use of a single feature to make a  choice (Fig. \ref{fig:ch2-model-selection}, models 1-4); six models that integrate two features (Fig. \ref{fig:ch2-model-selection}, models 5-10) and four models that integrate three features (Fig. \ref{fig:ch2-model-selection}, models 11-14).
...
Finally, to account for models implemented in an incomplete fashion, we created the models representing partial Tallying (Fig. \ref{fig:ch2-model-selection}, model 17-18) and incomplete TTB (Fig. \ref{fig:ch2-model-selection}, model 20-21). 
The specific formulas from each model listed in Fig. \ref{fig:ch2-model-selection} are included in Appendix A.
...
More specifically, we expected to find different patterns of T1-T2 performance change based on the underlying assumptions for each decision strategies as shown in Table \ref{tab:ch2-expectations}. 
...
### Strategy specific fixation patterns
...
Additionaly, the difference between the two most important features, compared to the two least informative features was not meaningful (F1 and F2 vs F2 and F4: `r report(rope_first_domain, strategy = "Partial Tallying", comparison = "d12_34")`). 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{../../figures/ch02_model_selection.png}
    \caption[Model classification.]{The bars represent the number of participants classified into each of the different decision strategies. The circles represent the features consulted by each model. In models 1-15, filled circles indicate the feature(s) used by each strategy. For models 16-18 we created a model that integrated the features in a single piece of information (i.e., Tallying). Models 19-21 consider a Serial Search for information. On the bottom we show the final list of decision models we used for the rest of the analysis.}
    \label{fig:ch2-model-selection}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{../../figures/ch02_score_panel.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{../../figures/ch02_rt_panel.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{../../figures/ch02_fixcount_panel.png}
    \caption[Visual information sampling.]{We considered the total number of fixations directed to any of the cue features as a measure of the amount of sampling done in each trial. (A) Posterior estimates of the number of fixations per trial during T1 across different strategies. (B) Paired comparisons between strategies' posterior distributions of fixation counts. Gray dots represent individual estimates. Central dots represent posterior medians in A and median differences in B. Error bars in all panels represent 89\% HDIs. The ROPE for the comparisons is shown as a shaded area in B.}
    \label{fig:ch2-fixcount}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{../../figures/ch02_fixcount_scenario_panel.png}
    \caption[Visual information sampling in Serial Search users across scenarios.]{In this case, we grouped trials depending on the decision scenarios illustrated earlier, using the total number of fixations directed to any of the four features in each trial by Serial Search users. (A) Posterior estimates of the number of fixations per trial during T1 across different decision scenarios. (B) Paired comparisons between scenarios' posterior distributions of fixation counts. Gray dots represent individual estimates. Central dots represent posterior medians in A and median differences in B. Error bars in all panels represent 89\% HDIs. The ROPE for the comparisons is shown as a shaded area in B.}
    \label{fig:ch2-fixcount-scenario}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{../../figures/ch02_fixation_patterns.png}
    \caption[Fixation patterns of each decision strategy.]{Left and center column show probability estimates of directing the first/last fixation towards any of the four features (F1-F4). Rightmost column displays overall allocation of fixations across features. Gray dots represent individual estimates, central points with error bars represent posterior medians with 89\% HDIs.}
    \label{fig:ch2-fixation-patterns}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{../../figures/ch02_serial_last.png}
    \caption[Stopping point across scenarios for Serial Search.]{These users implement a dynamic stopping rule depending on the location of the discriminationg feature. To evaluate this, we estimated the probability of directing the last fixation in a trial towards the different features (F1-F4), across decision scenarios. (A) Posterior estimates of the last fixation probabilities for each feature across decision scenarios. (B) Feature contrasts applied to the posterior distributions for each scenario. Gray dots represent individual estimates. Central dots represent posterior medians in A and median differences in B. Error bars in all panels represent 89\% HDIs. The ROPE for the comparisons is shown as a shaded area in B.}
    \label{fig:ch2-serial-last}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{../../figures/ch02_serial_last_comparison.png}
    \caption[Serial Search stopping point contrast across strategies.]{We applied the same feature contrasts used to test the Serial Search stopping point assumptions to all other strategies. Central dots represent median contrast value and intervals 89\% HDIs. The ROPE for the contrasts is shown as a shaded area.}
    \label{fig:ch2-serial-last-comparison}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{../../figures/ch02_interactions.png}
    \caption[Comparison of fixation patterns across strategies.]{Each line represents the overall gaze distribution across features for each of the five strategies reviewed in this study. The diamonds represent mean probability for each feature within a specific strategy.}
    \label{fig:ch2-interactions}
\end{figure}
