...
### Relative difference
These models use the ration of gamble features as decision criterion.
...
$$\begin{aligned} 
    \text{Model 13:}& \\
    left{[i]} & \sim \beta_{0} + \beta_{1} \times \text{Probability Winner} + \beta_{2} \times  \text{Reward Winner} \\
    \text{Model 14:}& \\
...
### Probability of choosing the highest EV gamble
...
For all the fixation pattern analyses, we assume that the number of feature fixations came from a multinomial distribution, with four different possible outcomes, accounting for the two pieces of information of both gambles (i.e., F1, F2, F3, F4). 
This multinomila model use the total number of feature fixations fixations (*n*) and a vector of event probabilities, i.e., the probabilities of fixating on a given feature (${\theta}^{F_i}$). 
